...
def export_napari_to_movie(fname, dt=None, fps=8,bitrate=1800,timestamp_position=[10,20],timestamp_color='white'):
    """
    ...
    for i in range(viewer.dims.range[0][1].astype(int)+1):
        ...
        if dt:
            t1=plt.text(timestamp_position[0], timestamp_position[1],"%02d:%02d"%(int(i*dt/60),i*dt%60),color=timestamp_color)
            ims.append([t1, im])
        else:
            ims.append([im])
